...
import { columnsAdministrativos, dataAdministrativos } from '../assets/constants/TablaDashboard'
    ...
    return (
        <div className='flex-1 overflow-auto relative z-10'>
            ...
            <main className='max-w-7xl mx-auto py-6 px-4 lg:px-8'>
                ...
                >
                    {DataCardAdministrativo.map((card, index) => (
                        <DashBoardCard
                            key={index}
                            nombre={card.nombre}
                            icon={card.icon}
                ...
                >
                    <div className='grid grid-cols-1 md:grid-cols-2 gap-6 sm:grid-cols-1'>
                        <div className='bg-white bg-opacity-50 backdrop-blur-md overflow-hidden shadow-lg rounded-xl p-6'>
                            ...
                            <Tabla data={dataAdministrativos} columns={columnsAdministrativos} />

                        </div>
                        <div className='bg-white bg-opacity-50 backdrop-blur-md overflow-hidden shadow-lg rounded-xl p-6'>
                            <h1 className='text-2xl font-bold py-4'>Mayor índice de administrativos</h1>
                            <PieChartBoard orderStatusData={orderAdministrativos} COLORS={COLORSAdministradores} />
                        </div>
                    </div>
                </motion.div>
            </main>
